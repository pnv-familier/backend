...
public class UnifiedDetectionService {
        ...
        public Mono<UnifiedDetectionResult> detectUnified(String message) {
                ...
                return callDetection(prompt, message, "gemini-2.5-flash-lite")
                                .onErrorResume(e -> {
                                        if (e instanceof org.springframework.web.reactive.function.client.WebClientResponseException.TooManyRequests) {
                                                log.warn("[Detection] 429 on gemini-2.5-flash-lite, falling back to gemini-3.1-flash-lite-preview");
                                                return callDetection(prompt, message, "gemini-3.1-flash-lite-preview");
                                        }
                                        log.error("Error in unified detection", e);
                                        return Mono.just(createDefaultResult());
                                });
        }

        private Mono<UnifiedDetectionResult> callDetection(String prompt, String originalMessage, String model) {
                String url = "/v1beta/models/" + model + ":generateContent?key=" + API_KEY;
                ...
                return webClient.post()
                                ...
                                .map(response -> parseUnifiedResponse(response, originalMessage))
                                .doOnNext(result -> log.info("[Detection][{}] mention={}, suggestion={}",
                                                model,
                                                result.getMention().isHasMention(),
                                                result.getSuggestion().isHasSuggestion()));
        }
        ...
        private UnifiedDetectionResult parseUnifiedResponse(Map<String, Object> response, String originalMessage) {
                try {
                        ...
                        UnifiedDetectionResult.MentionDetection mention = UnifiedDetectionResult.MentionDetection
                                        .builder()
                                        .hasMention(mentionNode.has("hasMention")
                                                        && mentionNode.get("hasMention").asBoolean())
                                        .targetRelation(mentionNode.has("targetRelation")
                                                        && !mentionNode.get("targetRelation").isNull()
                                                                        ? mentionNode.get("targetRelation").asText()
                                                                        : null)
                                        .confidence(mentionNode.has("confidence")
                                                        ? mentionNode.get("confidence").asDouble()
                                                        : 0.0)
                                        .build();
                        ...
                        String type = suggestionNode.has("type") && !suggestionNode.get("type").isNull()
                                        ? suggestionNode.get("type").asText()
                                        : null;
                        double confidence = suggestionNode.has("confidence")
                                        ? suggestionNode.get("confidence").asDouble()
                                        : 0.0;
                        ...
                        UnifiedDetectionResult.SuggestionDetection suggestion = UnifiedDetectionResult.SuggestionDetection
                                        .builder()
                                        .hasSuggestion(suggestionNode.has("hasSuggestion")
                                                        && suggestionNode.get("hasSuggestion").asBoolean())
                                        .type(type)
                                        .subType(suggestionNode.has("subType")
                                                        && !suggestionNode.get("subType").isNull()
                                                                        ? suggestionNode.get("subType").asText()
                                                                        : null)
